...
if [[ ${(%):-%m} = stormsend ]]; then
    ...
    # Staging tunnels.
    alias tunnelStg='ssh -L 18986:solr:8986 `#Keywords` -L 18987:solr:8987 `#NER` -L 18994:solr:8994 `#SpacesNew` -L 18993:solr:8993 `#Answers` -L 18995:solr:8995 `#UsersNew` -L 18988:solr:8988 `#Ads` -L 13306:mysql-master:3306 `#mysql` -L 16379:redis:6379 `#redis_main_and_feed` -L 18379:redis:18379 `#redis_cached_houses` stghouzz.hza'
